...
export async function generateMetadata({params}: LocalePageProps) {
  ...
  const page = await fetchLocalizedPage(locale, "services");
  if (!page) return {title: "Services | SS PLUS"};
  return {
    title: page.seo?.metaTitle || `${page.title} | SS PLUS`,
    description: page.seo?.metaDescription || "Our Services",
  };
}
...
export default async function LocalizedServicesPage({params}: LocalePageProps) {
    ...
    return (
      <main className="flex flex-1 items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <h1 className="font-display text-4xl font-bold mb-4 text-ss-pure-white uppercase">Services</h1>
          <p className="text-ss-grey-400 max-w-md mx-auto">
            The Services page for "{locale}" has not been published in Sanity yet.
          </p>
        </div>
      </main>
    );
  ...
  return (
    <main className="pt-24 min-h-screen">
      <BlockRenderer blocks={page.content} pageSlug="services" />
    </main>
  );
}
