...
type Locale = "en" | "fr";
type PageKey = "home" | "about" | "services" | "products" | "qhse" | "wave-plus" | "contact";
...
function switchLocalePath(pathname: string, targetLocale: Locale): string {
  ...
  const matchedKey = pageKeys.find((key) => slugsByLocale[currentLocale][key] === currentFirst) ?? "home";
  ...
  return parts.length ? `/${targetLocale}/${parts.join("/")}` : `/${targetLocale}`;
}
  ...
  return (
    ...
    >
      <div className="ss-container flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          <Link
            ...
            className="group relative inline-flex items-center px-8 py-3 bg-ss-red/5 text-ss-red border border-ss-red/20 font-bold text-[11px] uppercase tracking-[0.2em] rounded-sm transition-all duration-500 hover:bg-ss-red hover:text-[#fff] hover:shadow-[0_0_30px_rgba(214,40,40,0.3)] overflow-hidden"
          >
        ...
        <div className="relative z-110 flex lg:hidden items-center gap-2">
          ...
          >
            <span className={`w-6 h-0.5 bg-white transition-all duration-300 ${isMenuOpen ? "rotate-45 translate-y-2 bg-ss-red" : ""}`} />
      ...
      >
        ...
        >
          <div className="absolute inset-0 ss-noise opacity-20" />
          <div className="absolute top-0 right-0 w-full h-full bg-[radial-gradient(circle_at_80%_20%,rgba(214,40,40,0.1)_0%,transparent_50%)]" />

          <div className="relative min-h-full flex flex-col pt-24 px-6 sm:px-8 pb-8" ref={menuLinksRef}>
            ...
            {navLinks.map((item) => (
              <Link
                ...
                href={item.href}
                onClick={() => setIsMenuOpen(false)}
                className="block rounded-sm px-3 py-3 text-xl sm:text-2xl font-display font-bold text-white hover:text-ss-red hover:bg-white/[0.03] transition-colors duration-300 tracking-tight"
              >
                {item.name}
              </Link>
            ))}
            ...
            <div className="mt-8 pt-6 border-t border-white/10 flex flex-col gap-5">
              ...
              <Link
                href={toLocalizedHref(currentLocale, "contact")}
                onClick={() => setIsMenuOpen(false)}
                className="inline-flex items-center justify-center px-6 py-4 bg-ss-red text-[#fff] font-bold text-xs uppercase tracking-[0.18em] rounded-sm hover:shadow-[0_0_40px_rgba(214,40,40,0.4)] transition-all duration-300"
              >
                {t.contact}
              </Link>

              <div className="flex flex-col gap-2">
                <span className="text-[10px] font-bold text-ss-grey-400 uppercase tracking-widest">{t.emergencyHotline}</span>
                <a href="tel:+21628308108" className="text-lg font-display font-bold text-white hover:text-ss-red transition-colors">
                  +216 28 308 108
                </a>
              </div>
            </div>
